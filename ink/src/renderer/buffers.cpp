...
ElementBuffer::ElementBuffer() { glGenBuffers(1, &h_rendererID); }

ElementBuffer::~ElementBuffer() { glDeleteBuffers(1, &h_rendererID); }
...
uint32_t ElementBuffer::GetCount() const { return count; }
...
ElementBuffer *ElementBuffer::Create(std::vector<uint32_t> elements) {
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, eb->count * sizeof(uint32_t),
               &elements[0], GL_STATIC_DRAW);
  return eb;
}

VertexBuffer::VertexBuffer() { glGenBuffers(1, &h_rendererID); }

VertexBuffer::~VertexBuffer() { glDeleteBuffers(1, &h_rendererID); }

void VertexBuffer::Bind() const { glBindBuffer(GL_ARRAY_BUFFER, h_rendererID); }
